...
def main(args, env_name, number, seed):
    ...
    env_evaluate.seed(40)
    env_evaluate.action_space.seed(40)
    np.random.seed(seed)
    ...
    save_fn = 'reward_proposed_3.mat'
    sio.savemat(save_fn, {'reward_proposed_3': episode_rewards})

    reward_list = loadmat('reward_proposed_3.mat')
    reward_list = reward_list['reward_proposed_3']
    rolling_intv = 10
    df = pd.DataFrame(list(*reward_list))

    d = list(np.hstack(df.rolling(rolling_intv, min_periods=1).mean().values))
    save_fn = 'reward_proposed_3_smooth.mat'
    sio.savemat(save_fn, {'reward_proposed_smooth_3': d})


if __name__ == '__main__':
    parser = argparse.ArgumentParser("Hyperparameters Setting for PPO-continuous")
    parser.add_argument("--max_train_steps", type=int, default=int(500e3), help=" Maximum number of training steps")
    ...
    env_name = ['UCMEC']
    env_index = 0
    main(args, env_name=env_name[env_index], number=10, seed=100)
