# average allocate the computing resources
...
class UCMEC(gym.Env):
    ...
    def step(self, action):
        ...
        state = rate_calculate_old(p_current)  # Transmit Rate calculation 1xM
        ...
        # calculate the edge offloading delay

        # evenly allocate the computing resource
        c_current = np.zeros([M])
        offload_user_num = 0
        for i in range(M):
            if a_current[i] != 0:
                offload_user_num += 1
        if offload_user_num == 0:
            c_each_user = C_edge
        else:
            c_each_user = C_edge / offload_user_num
        for i in range(M):
            if a_current[i] != 0:
                c_current[i] = c_each_user
...
if __name__ == '__main__':
    ...
    env_name = ['UCMEC-Optimal-Offloading']
    env_index = 0
